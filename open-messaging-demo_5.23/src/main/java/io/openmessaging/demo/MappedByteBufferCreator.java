...
public class MappedByteBufferCreator {
	...
	public static MappedByteBuffer createCommitQueueBuffer(String storePath, String bucket){
		MappedByteBuffer buffer = null;
		try {
			String fullPathName = storePath + File.separator + Constants.COMMITQUEUE_FOLDER_NAME + 
					File.separator + bucket + "_offset.txt";
			buffer = MappedByteBufferCreator.createBuffer(fullPathName, 0,  Constants.COMMITQUEUE_FILE_MAX_LENGTH, false, FileChannel.MapMode.READ_ONLY);
		} catch (IOException e) {
			e.printStackTrace();
		}
		return buffer;
