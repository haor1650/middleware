...
public class MappedByteBufferCreator {
	...
	public static MappedByteBuffer createBuffer(String fullPathName, long maxLength, boolean createIfNotExist, MapMode mode) throws IOException{
		...
		try {
			...
			buffer = createBuffer(file, maxLength, createIfNotExist, mode);
		} catch (IOException e) {
			e.printStackTrace();
			throw e;
		}
	...
	public static MappedByteBuffer createBuffer(File commitLogFile, long maxLength, boolean createIfNotExist, MapMode mode) throws IOException{
		MappedByteBuffer buffer = null;
		try {
