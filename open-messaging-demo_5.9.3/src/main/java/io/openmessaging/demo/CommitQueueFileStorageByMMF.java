...
public class CommitQueueFileStorageByMMF {
	...
	public CommitQueueFileStorageByMMF(String storePath, String bucket){
	...
	public synchronized void storeCommitQueue(Long offsetAndLength){
	...
	private MappedByteBuffer createCommitQueueBuffer(String bucket){
		...
		try {
			...
			buffer = MappedByteBufferCreator.createBuffer(fullPathName,0, Constants.COMMITQUEUE_FILE_MAX_LENGTH, true, FileChannel.MapMode.READ_WRITE);
		} catch (IOException e) {
			e.printStackTrace();
		}
		return buffer;
